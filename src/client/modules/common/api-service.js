...
angular.module('koan.common').factory('api', function ($rootScope, $http, $window) {
  ...
  api.shouts = {
    list: function () {
      return $http({method: 'GET', url: apiBase + '/shouts', headers: headers});
    },
    create: function (shout) {
      return $http({method: 'POST', url: apiBase + '/shouts', data: shout, headers: headers});
    },
    created: event()
  };
  ...
  api.users = {
    update: function(user) {
      return $http({method: 'PUT', url: apiBase + '/users', data: user, headers: headers});
    }
  };
